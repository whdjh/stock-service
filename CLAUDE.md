...
### 📝 커밋 규칙
- **커밋 메시지는 한국어로 작성한다.**
- **prefix는 `feat`, `fix`, `chore`, `docs` 4가지만 사용한다.**
- 예: `feat: 대시보드 메인 페이지 조립`, `fix: 사이드바 반응형 레이아웃 수정`
...
- **프레임워크:** React 18 (SPA).
- **라우팅:** React Router v7.
- **다국어:** react-i18next (localStorage 기반 locale 저장).
...
4.  **페이지 조립:** `src/pages/HomePage.tsx`에 위 컴포넌트들을 배치하고 `mockData`를 연결하여 화면 완성.